...
// Entrance for the left column, in both scroll directions: whenever a block
// enters the viewport it rises/drops ~40px into place while a heavy blur clears
// and it fades in — from below when scrolling down, from above when scrolling
// back up (matching the reference videos). It is a slow, eased, time-based
// animation, and it replays every time the block re-enters. On leaving, the
// block resets instantly (it is off-screen) to the side it left by, so the
// next entrance starts from the right direction. Blocks already on screen at
// load animate in the same way. Reduced motion: shown as-is.
...
function Flow({ children, className }) {
  const reduceMotion = useReducedMotion();
